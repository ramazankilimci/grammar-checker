...
# File wsgi.py was not found in subfolder: 'grammar-checker'. Please enter the Python path to wsgi file.
# CMD ["gunicorn", "--bind", "0.0.0.0:80", "pythonPath.to.wsgi"]

CMD ["sh", "-c", "python manage.py makemigrations && python manage.py migrate && gunicorn --bind 0.0.0.0:80 swe599_project.wsgi"]
